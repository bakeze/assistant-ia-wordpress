...
final class ACE_Provider_OpenAI extends ACE_Provider_Base
{
    public function chat(array $payload): array
    {
        $key = trim((string) ($this->settings['openai_api_key'] ?? ''));
        if ($key === '') {
            return ['error' => __('OpenAI API key is missing.', 'ai-chat-editor')];
        }
